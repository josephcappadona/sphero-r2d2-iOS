...
public class DeathStarMaze {
    ...
    private static func parse(representation: String, name: String) -> (maze: DeathStarMaze, additionalPoints: [Character:CGPoint]) {
        ...
        let characterGrid = rowRepresentations.map { (rowRepresentation: String) -> [Character] in
            return Array<Character>(rowRepresentation.characters)
        }
        ...
        for (rowIndex,row) in characterGrid.enumerated() {
            for (colIndex,char) in row.enumerated() {
                ...
                maze.visionGrid[2*rowIndex][2*colIndex]     = "┼┷┨╆╄╅┛".characters.contains(char)
                maze.visionGrid[2*rowIndex][2*colIndex+1]   = "┼┠┷╆╃╅┗".characters.contains(char)
                maze.visionGrid[2*rowIndex+1][2*colIndex]   = "┼┯┨╆╄╃┓".characters.contains(char)
                maze.visionGrid[2*rowIndex+1][2*colIndex+1] = "┼┯┠╄╃╅┏".characters.contains(char)
    ...
    public static let finalMaze: DeathStarMaze = {
        ...
        for pathString in [
            ...
            "7456"
                ...
                var path = [CGPoint]()
                for char in pathString.characters {
                    path.append(pts[char]!)
                }
                maze.stormtrooperPaths.append(path)
        }
        
        for char in "qr".characters {
