...
extension SpheroV1Toy: DriveRollable {
    ...
    public func roll(heading: Double, speed: Double) {
        let direction: Roll.RollDirection = speed < 0 ? .reverse : .forward
        let doubleSpeed = direction == .reverse ? -speed : speed
        let intSpeed = UInt8(doubleSpeed.clamp(lowerBound: 0.0, upperBound: 255.0))
        let doubleHeading = direction == .reverse ? heading + 180.0 : heading
        let intHeading = UInt16(doubleHeading.positiveRemainder(dividingBy: 360.0))

        sendRollCommand(heading: intHeading, speed: intSpeed, rollType: .roll, direction: direction)
    }
