...
class R2D2Toy: SpheroV2Toy {
    ...
    public func setFrontPSILed(color: FrontPSIColor) {
        core.send(SetAllLEDCommand(ledMask: R2LEDMask.frontPSI, color: color.color()))
    }
    
    public func setBackPSILed(color: BackPSIColor) {
        core.send(SetAllLEDCommand(ledMask: R2LEDMask.backPSI, color: color.color()))
    }
    ...
    public func setHoloProjectorLed(brightness: Double) {
        ...
        core.send(SetAllLEDCommand(ledMask: R2LEDMask.holoProjector, brightness: clampedBrightness))
    }
    
    public func setLogicDisplayLeds(brightness: Double) {
        let clampedBrightness = UInt8(brightness.clamp(lowerBound: 0.0, upperBound: 255.0))
        core.send(SetAllLEDCommand(ledMask: R2LEDMask.logicDisplays, brightness: clampedBrightness))
    }
    
    public func setAudioVolume(_ volume: Int) {
        let clampedVolume = UInt8(truncatingBitPattern: volume)
    ...
    public override var batteryLevel: Double? {
        get {
            ...
            return (batteryVoltage - 3.3) / (4.00 - 3.3)
        }
    }
    ...
    override func roll(heading: Double, speed: Double) {
        switch stanceState {
        case .tripod:
