...
class BB9EToy: SpheroV2Toy {
    ...
    public func setMainLed(color: UIColor) {
        core.send(SetAllLEDCommand(ledMask: BB9ELEDMask.body, color: color))
    }
    
    public func setBackLed(brightness: Double) {
        let clampedBrightness = UInt8(brightness.clamp(lowerBound: 0.0, upperBound: 255.0))
        core.send(SetAllLEDCommand(ledMask: BB9ELEDMask.back, brightness: clampedBrightness))
    }
    
    public func setHeadLed(brightness: Double) {
        let clampedBrightness = UInt8(brightness.clamp(lowerBound: 0.0, upperBound: 255.0))
        core.send(SetAllLEDCommand(ledMask: BB9ELEDMask.head, brightness: clampedBrightness))
    }
