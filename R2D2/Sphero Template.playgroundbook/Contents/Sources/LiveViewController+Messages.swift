...
extension LiveViewController: PlaygroundLiveViewMessageHandler {
    ...
    public func receive(_ message: PlaygroundValue) {
        ...
        case .setBackLedColor:
            sendSetBackLedColor(dict: dict)
            
        case .setFrontLedColor:
            sendSetFrontLedColor(dict: dict)

        case .drawMatrixPixelColor:
            sendDrawMatrixPixelColor(dict: dict)

        case .drawMatrixLineColor:
            sendDrawMatrixLineColor(dict: dict)

        case .drawMatrixFromStartPixelToEndPixelWithColor:
            sendDrawMatrixFromStartPixelToEndPixelWithColor(dict: dict)

        case .setMatrixColor:
            sendSetMatrixColor(dict: dict)

        case .clearMatrix:
            sendClearMatrix()

        case .setMatrixRotation:
            sendSetMatrixRotation(dict: dict)

        case .scrollMatrixText:
            sendScrollMatrixText(dict: dict)
    ...
    func sendSensorDataMessage(data: SensorControlData) {
        ...
        if let gyro = data.gyro {
            ...
            if let filtered = gyro.rotationRate {
                ...
                if let x = filtered.x {
                    filteredDict[MessageKeys.x] = .integer(x)
                }
                if let y = filtered.y {
                    filteredDict[MessageKeys.y] = .integer(y)
                }
                if let z = filtered.z {
                    filteredDict[MessageKeys.z] = .integer(z)
                }
    ...
    func sendSetBackLedColor(dict: MessageDict) {
        guard let color = dict[MessageKeys.backLedColor]?.colorValue() else { return }
        
        connectedToy?.setBackLed(color: color)
    }
    
    func sendSetFrontLedColor(dict: MessageDict) {
        guard let color = dict[MessageKeys.frontLedColor]?.colorValue() else { return }
        
        connectedToy?.setFrontLed(color: color)
    }
    
    func sendDrawMatrixPixelColor(dict: MessageDict) {
        guard let pixelX = dict[MessageKeys.pixelX]?.intValue(), let pixelY = dict[MessageKeys.pixelY]?.intValue(), let color = dict[MessageKeys.pixelLedColor]?.colorValue() else { return }
        
        let pixel = Pixel(x: pixelX, y: pixelY)
        
        connectedToy?.drawMatrix(pixel: pixel, color: color)
    }
    
    func sendDrawMatrixLineColor(dict: MessageDict) {
        guard let startPixelX = dict[MessageKeys.startPixelX]?.intValue(),
            let startPixelY = dict[MessageKeys.startPixelY]?.intValue(),
            let endPixelX = dict[MessageKeys.endPixelX]?.intValue(),
            let endPixelY = dict[MessageKeys.endPixelY]?.intValue(),
            let color = dict[MessageKeys.pixelLedColor]?.colorValue() else { return }
        
        let startPixel = Pixel(x: startPixelX, y: startPixelY)
        let endPixel = Pixel(x: endPixelX, y: endPixelY)
        
        connectedToy?.drawMatrixLine(from: startPixel, to: endPixel, color: color)
    }
    
    func sendDrawMatrixFromStartPixelToEndPixelWithColor(dict: MessageDict) {
        guard let startPixelX = dict[MessageKeys.startPixelX]?.intValue(), let startPixelY = dict[MessageKeys.startPixelY]?.intValue(),
        let endPixelX = dict[MessageKeys.endPixelX]?.intValue(), let endPixelY = dict[MessageKeys.endPixelY]?.intValue(),
        let color = dict[MessageKeys.pixelLedColor]?.colorValue() else { return }
    ...
    func sendSetMatrixColor(dict: MessageDict) {
        guard let color = dict[MessageKeys.pixelLedColor]?.colorValue() else { return }
        
        connectedToy?.setMatrix(color: color)
    }
    
    func sendClearMatrix() {
        connectedToy?.clearMatrix()
    }
    
    func sendSetMatrixRotation(dict: MessageDict) {
        guard let rotationValue = dict[MessageKeys.rotationValue]?.doubleValue(),
            let rotation = MatrixRotation(rawValue: UInt8(rotationValue)) else { return }
        
        connectedToy?.setMatrix(rotation: rotation)
    }
    
    func sendScrollMatrixText(dict: MessageDict) {
        guard let text = dict[MessageKeys.textToDisplay]?.stringValue(),
        let color = dict[MessageKeys.textColor]?.colorValue(),
        let speed = dict[MessageKeys.speedOfText]?.intValue(),
        let loopMode = dict[MessageKeys.loopMode]?.doubleValue(),
        let scrollingTextLoopMode = ScrollingTextLoopMode(rawValue: UInt8(loopMode))
        else { return }
        
        connectedToy?.scrollMatrix(text: text, color: color, speed: speed, loop: scrollingTextLoopMode)
    }
