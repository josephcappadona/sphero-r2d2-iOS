...
public enum MessageTypeId: Int {
    ...
    case setHeadLed = 160
    case setBackLed = 162
    case setBackLedColor = 164
    case setFrontLedColor = 165
    case drawMatrixPixelColor = 166
    case drawMatrixLineColor = 167
    case drawMatrixFromStartPixelToEndPixelWithColor = 168
    case setMatrixColor = 169
    case clearMatrix = 172
    case setMatrixRotation = 173
    case scrollMatrixText = 174
...
public enum MessageKeys {
    ...
    public static let backLedColor = "backLedColor"
    public static let frontLedColor = "frontLedColor"
    public static let pixelLedColor = "pixelLedColor"
    public static let matrixColor = "matrixColor"
    public static let textColor = "textColor"
    public static let pixelX = "pixelX"
    public static let pixelY = "pixelY"
    public static let startPixelX = "startPixelX"
    public static let startPixelY = "startPixelY"
    public static let endPixelX = "endPixelX"
    public static let endPixelY = "endPixelY"
    public static let rotationValue = "rotationValue"
    public static let textToDisplay = "textToDisplay"
    public static let speedOfText = "speedOfText"
    public static let loopMode = "loopMode"
...
public class ToyMessageSender: ToyCommandListener {
    ...
    public func setBackLed(color: UIColor) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.setBackLedColor.playgroundValue(),
            MessageKeys.backLedColor: PlaygroundValue(color: color)
        ]))
    }
    
    public func setFrontLed(color: UIColor) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.setFrontLedColor.playgroundValue(),
            MessageKeys.frontLedColor: PlaygroundValue(color: color)
        ]))
    }
    
    public func drawMatrix(pixel: Pixel, color: UIColor) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.drawMatrixPixelColor.playgroundValue(),
            MessageKeys.pixelX: .integer(pixel.x),
            MessageKeys.pixelY: .integer(pixel.y),
            MessageKeys.pixelLedColor: PlaygroundValue(color: color)
        ]))
    }
    
    public func drawMatrixLine(from startPixel: Pixel, to endPixel: Pixel, color: UIColor) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.drawMatrixLineColor.playgroundValue(),
            MessageKeys.startPixelX: .integer(startPixel.x),
            MessageKeys.startPixelY: .integer(startPixel.y),
            MessageKeys.endPixelX: .integer(endPixel.x),
            MessageKeys.endPixelY: .integer(endPixel.y),
            MessageKeys.pixelLedColor: PlaygroundValue(color: color)
            ]))
    }
    ...
    public func drawMatrix(fillFrom startPixel: Pixel, to endPixel: Pixel, color: UIColor) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.drawMatrixFromStartPixelToEndPixelWithColor.playgroundValue(),
            MessageKeys.startPixelX: .integer(startPixel.x),
            MessageKeys.startPixelY: .integer(startPixel.y),
            MessageKeys.endPixelX: .integer(endPixel.x),
            MessageKeys.endPixelY: .integer(endPixel.y),
    ...
    public func setMatrix(color: UIColor) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.setMatrixColor.playgroundValue(),
            MessageKeys.pixelLedColor: PlaygroundValue(color: color)
        ]))
    }
    ...
    public func clearMatrix() {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.clearMatrix.playgroundValue(),
        ]))
    }
    
    public func setMatrix(rotation: MatrixRotation) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.setMatrixRotation.playgroundValue(),
            MessageKeys.rotationValue: .floatingPoint(Double(rotation.rawValue))
        ]))
    }
    
    public func scrollMatrix(text: String, color: UIColor, speed: Int, loop: ScrollingTextLoopMode) {
        send(.dictionary([
            MessageKeys.type: MessageTypeId.scrollMatrixText.playgroundValue(),
            MessageKeys.textColor: PlaygroundValue(color: color),
            MessageKeys.textToDisplay: .string(text),
            MessageKeys.speedOfText: .integer(speed),
            MessageKeys.loopMode: .floatingPoint(Double(loop.rawValue))
        ]))
    }
